...
func GetGitDiffOutput(baseBranch string, headBranch string, reviewIgnorePath string) ([]byte, error) {
	...
	cmdArgs := []string{"git", "diff", fmt.Sprintf("origin/%s", baseBranch), fmt.Sprintf("origin/%s", headBranch), "--", "."}
	if len(patterns) > 0 {
		var args []string
...
func GetChatGptResponse(endpoint string, model string, apiKey string, diff []byte, language string) ([]byte, error) {
	chatGPTRequest := ChatGPTRequest{
		...
		Messages: []Prompt{
			{
				Content: fmt.Sprintf("You are an excellent software engineer. Please review the output of the following `git diff` and propose some refactoring. Please provide your response in %s using bullet points.\n```\n%s\n```", language, string(diff)),
				Role:    "user",
			},
		},
	}
